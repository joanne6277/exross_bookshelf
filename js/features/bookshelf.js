        ...
        if (resources) {
            if (resources.attachments && resources.attachments.length > 0) {
                ...
                resources.attachments.forEach(file => {
                    contentHTML += `<a href="${file.url}" class="flex items-center justify-between p-3 bg-gray-50 rounded-lg border border-border-color hover:bg-gray-100 transition-colors"><div class="flex items-center gap-3"><i data-lucide="file-text" class="w-5 h-5 text-text-secondary"></i><div><div class="font-medium text-text-primary">${file.name}</div><div class="text-xs text-text-secondary">${file.size}</div></div></div><div class="flex items-center gap-2"><i data-lucide="download" class="w-5 h-5 text-text-secondary"></i></div></a>`;
                });
                contentHTML += '</div>';
            }
            if (resources.links && resources.links.length > 0) {
                ...
                resources.links.forEach(link => {
                    contentHTML += `<a href="${link.url}" target="_blank" class="flex items-center gap-3 p-3 bg-gray-50 rounded-lg border border-border-color hover:bg-gray-100 transition-colors"><i data-lucide="link" class="w-5 h-5 text-text-secondary"></i><span class="font-medium text-text-primary">${link.title}</span><div class="flex items-center gap-2 ml-auto"><i data-lucide="arrow-up-right" class="w-4 h-4 text-text-secondary"></i></div></a>`;
                });
                contentHTML += '</div>';
            }
        }
        ...
        let filtered = fullData.filter(book => {
            ...
            if (activeFilters.source !== '全部' && activeFilters.source !== '全部來源') {
                if (!book.source.includes(activeFilters.source) && activeFilters.source !== book.source) {
                    ...
                    if (target.includes('灰熊愛讀書')) target = '灰熊愛讀書';
